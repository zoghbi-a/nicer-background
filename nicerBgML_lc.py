...
if __name__ == '__main__':
    ...
    try:
        averageR  = pd.DataFrame(modData['average_rate'])
    except KeyError:
        raise KeyError(f'Model "{modelFile}" does not support light curves')
    print('... Done'); print('-'*20)
    ...
    rates   = averageR.iloc[gPred]
    rates.insert(0, 'time', time)
    lctxt = '# time rate_0.2-0.4 rate_0.4-15\n'
    lctxt += '\n'.join(['{:10.10} {:6.6} {:6.6}'.format(*a) for a in rates.values])
